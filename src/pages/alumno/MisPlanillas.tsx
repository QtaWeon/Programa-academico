...
const MisPlanillas = () => {
  ...
  // Institutional tab: show ALL institutional planillas for the year (no month filter — one per etapa)
  const isInstTab = taskFilter === 'institucional';

  const visiblePlanillas = planillas.filter(p => {
    if (p.status !== 'aprobado') return false;
    if (!p.scores.some(s => s.studentId === STUDENT_ID)) return false;
    if (p.year !== CURRENT_YEAR) return false;

    if (isInstTab) {
      // Show all institutional planillas regardless of month
      return p.planillaType === 'institucional';
    }

    // Normal tabs: filter by selected month
  ...
  const renderPlanillaCard = (planilla: Planilla) => {
    const myScores = planilla.scores.find(s => s.studentId === STUDENT_ID);
    if (!myScores) return null;

    const tasks = planilla.tasks;
    const totalMax = tasks.reduce((s, t) => s + t.maxPoints, 0);
    const myTotal = tasks.reduce((s, t) => s + (myScores.scores[t.id] || 0), 0);

    const pct = totalMax > 0 ? myTotal / totalMax : 0;
    const colorClass = pct >= 0.8 ? 'text-green-600' : pct >= 0.5 ? 'text-amber-600' : 'text-red-600';
    const barClass = pct >= 0.8 ? 'bg-green-500' : pct >= 0.5 ? 'bg-amber-500' : 'bg-red-500';
        ...
        <CardContent className="p-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {isInst ? `Coordinador: ${planilla.teacherName}` : `Prof. ${planilla.teacherName}`} · Máx: {totalMax} pts
              </p>
            </div>
            <div className="flex flex-col items-start sm:items-end gap-1">
              <Badge className="bg-green-500/20 text-green-700 border-green-300">Publicado</Badge>
              {!myClaim && canClaim && !isInst && (
                <Button variant="outline" size="sm" className="h-6 text-xs mt-1 border-yellow-300 text-yellow-700 bg-yellow-50 hover:bg-yellow-100"
                  onClick={() => { setClaimDialogPlanilla(planilla); setClaimText(''); }}>
              ...
              {myClaim && !myClaim.resolved && (
                <Button variant="outline" size="sm" className="h-6 text-xs mt-1 border-blue-300 text-blue-700 bg-blue-50"
                  onClick={() => setViewClaim(myClaim)}>
                  Reclamo Pendiente
                </Button>
              )}
              {myClaim && myClaim.resolved && (
                <Button variant="outline" size="sm" className="h-6 text-xs mt-1 border-green-300 text-green-700 bg-green-50"
                  onClick={() => setViewClaim(myClaim)}>
                  Reclamo Resuelto
                </Button>
              )}
          ...
          <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-2 mb-3">
            {tasks.map(task => (
              <div key={task.id} className="text-center border rounded-lg p-1.5 sm:p-2 min-w-0 bg-background/60 flex flex-col justify-between h-14">
                <div className="text-[10px] text-muted-foreground truncate px-1" title={task.name === 'Puntaje de Clubes' ? 'Clubes' : task.name}>
                  {task.name === 'Puntaje de Clubes' ? 'Clubes' : task.name}
                </div>
                <div className="flex items-baseline justify-center gap-0.5">
                  <span className="font-bold text-xs sm:text-sm">{myScores.scores[task.id] || 0}</span>
                  <span className="text-[10px] text-muted-foreground">/ {task.maxPoints}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 mt-4">
            <div className="flex items-center gap-4 w-full sm:w-auto">
              <div className={`text-3xl font-bold ${colorClass}`}>{myTotal}</div>
              <div className="text-sm text-muted-foreground">/ {totalMax} puntos</div>
            </div>
            <div className="flex-1 w-full">
              <div className="h-2 bg-muted rounded-full overflow-hidden">
                <div className={`h-full rounded-full ${barClass}`} style={{ width: `${Math.min(pct * 100, 100)}%` }} />
              </div>
            ...
            {pct < 0.5 && (
              <div className="flex w-full sm:w-auto items-center gap-1 text-xs text-red-600 mt-2 sm:mt-0 bg-red-50 p-2 rounded">
                <AlertTriangle className="h-3 w-3" /> Consulte con un profesor.
              </div>
            )}
          </div>
        </CardContent>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        {/* Month selector — hidden for institutional tab */}
        {!isInstTab && (
      ...
      <div className="text-center bg-primary/10 border border-primary/20 rounded-lg p-3">
        <h3 className="font-bold text-lg">
          {isInstTab
            ? `Puntajes Institucionales ${CURRENT_YEAR}`
            : `Puntajes de ${monthName} ${CURRENT_YEAR}`}
        </h3>
        <p className="text-sm text-muted-foreground">
          {studentName}{grade ? ` — ${grade} Bachillerato Técnico en Informática` : ''}
        </p>
        {isInstTab && (
          <p className="text-xs text-muted-foreground mt-1">
            Clubes · Asistencia · Puntualidad — Se registra una vez por etapa
          </p>
        )}
      </div>
      ...
      {!loading && visiblePlanillas.length === 0 && (
        <p className="text-center text-muted-foreground py-8">
          {isInstTab
            ? 'No hay puntajes institucionales registrados para este año.'
            : 'No hay planillas aprobadas para este mes.'}
        </p>
      )}

      {!loading && visiblePlanillas.map(p => renderPlanillaCard(p))}
