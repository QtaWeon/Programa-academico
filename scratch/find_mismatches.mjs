...
async function findMismatches() {
  ...
  console.log(`Analyzing ${students.length} students across ${approvedPlanillas.length} approved planillas.`);

  students.forEach(student => {
    const STUDENT_ID = student.id;
    const myPlanillas = approvedPlanillas.filter(p => p.scores.some(s => s.studentId === STUDENT_ID));
    if (myPlanillas.length === 0) return;

    // Etapa 1
    const monthFilter = [2, 3, 4, 5];
    const instColumns = {};
    ...
    myPlanillas
      .filter(p => monthFilter.includes(p.month) && p.planillaType !== 'institucional')
      .forEach(planilla => {
        const { subjectId, subjectName, tasks, scores } = planilla;
    ...
    // Check if student has any subject where grade is calculated on totalMaxPoints === 0
    Object.keys(stats).forEach(sid => {
      const s = stats[sid];
      if (s.totalMaxPoints === 0) {
        console.warn(`Mismatch: Student ${student.lastName}, ${student.firstName} has 0 max points for subject ${s.subjectName}`);
      }
      
      // Let's check if the student points exceed max points
      if (s.totalStudentPoints > s.totalMaxPoints) {
        console.warn(`CRITICAL: Student ${student.lastName}, ${student.firstName} has student points ${s.totalStudentPoints} exceeding max points ${s.totalMaxPoints} in subject ${s.subjectName}!`);
      }
    });
  });
